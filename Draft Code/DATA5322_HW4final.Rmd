...
```{r}
#km_df2010num$centers
```

```{r}
country_cluster2010 <- data.frame(country = df2010$country, cluster = km_df2010num$cluster)
...
```{r}
country_cluster <- data.frame(country = df2010$country, cluster = km_df2010num$cluster)
...
```{r}
region_cluster <- data.frame(country = df2010$region, cluster = km_df2010num$cluster)
...
```{r}
#cluster_split2000 <- split(country_region_cluster2000$country, country_cluster$cluster)

# Tabulate countries for each cluster using lapply
#tabulated_clusters2000 <- lapply(cluster_split2000, table)

# Print the tabulated result for each cluster
#for (i in seq_along(tabulated_clusters2000)) {
  #cluster_number <- names(tabulated_clusters2000)[i]
 # cat("Cluster", cluster_number, ":\n")
 # print(tabulated_clusters[[i]])
 # cat("\n")
 # cluster_filename <- paste0("cluster_", cluster_number, ".csv")
#  write.csv(cluster_table, file = cluster_filename, row.names = TRUE)
